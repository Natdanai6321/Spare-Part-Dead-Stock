...
?>
  ...
  <?php
        ...
        if($val =='' AND $Mfg_val =='MFG'){
            $sql = "SELECT * FROM tbl_borrow_sparepart WHERE  B_userID='$m_userID' ORDER BY B_id ASC  LIMIT $offset,$record_show" or die("Error:" . mysqli_error());

        }
        else if(($val =='' AND $Mfg_val !='MFG' )){
            $sql = "SELECT * FROM tbl_borrow_sparepart
            WHERE B_Mfg LIKE '%$Mfg_val%'AND B_userID='$m_userID' ORDER BY B_id ASC   LIMIT $offset,$record_show";
 
        }
        else if(($val !='' AND $Mfg_val =='MFG')){
            $sql = "SELECT * FROM tbl_borrow_sparepart
            WHERE B_user LIKE '%$val%' OR B_Item_number LIKE '%$val%' OR B_Item_name LIKE '%$val%' 
            OR date_borrow LIKE '%$val%'OR date_return LIKE '%$val%'OR date_return_True LIKE '%$val%'AND B_userID='$m_userID' ORDER BY B_id ASC  LIMIT $offset,$record_show";
           
        }
        else if(($val !='' AND $Mfg_val !='MFG' )){
            $sql = "SELECT * FROM tbl_borrow_sparepart
            WHERE B_user LIKE '%$val%'  OR B_Item_number LIKE '%$val%' OR B_Item_name LIKE '%$val%' 
            OR date_borrow LIKE '%$val%'OR date_return LIKE '%$val%'OR date_return_True LIKE '%$val%' AND  B_Mfg LIKE '%$Mfg_val%'
            AND B_userID='$m_userID' ORDER BY B_id ASC  LIMIT $offset,$record_show";
           }
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                    <?php do { ?>
                                    <?php if($m_userID==$row_am['B_userID']){?>
                                    <tr>
