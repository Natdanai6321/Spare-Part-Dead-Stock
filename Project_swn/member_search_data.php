...
?>
  ...
  <?php
        ...
        if($val =='' AND $Mfg_val =='MFG' AND $Status_val =='STATUS'){
            ...
            WHERE Status LIKE '%$Status_val%' ORDER BY Item_number ASC   LIMIT $offset,$record_show";
             $state = '2';
        }
        else if(($val =='' AND $Mfg_val !='MFG' AND $Status_val =='STATUS')){
            $sql = "SELECT * FROM tbl_spare_part
            WHERE Mfg_id LIKE '%$Mfg_val%' ORDER BY Item_number ASC  LIMIT $offset,$record_show";
                     $state = '3';
        }
        else if(($val =='' AND $Mfg_val !='MFG' AND $Status_val !='STATUS')){
            $sql = "SELECT * FROM tbl_spare_part
            WHERE Mfg_id LIKE '%$Mfg_val%' AND
                  Status LIKE '%$Status_val%' ORDER BY Item_number ASC  LIMIT $offset,$record_show";
                   $state = '4';
        }
        else if(($val !='' AND $Mfg_val =='MFG' AND $Status_val =='STATUS')){
            $sql = "SELECT * FROM tbl_spare_part
            WHERE Item_number LIKE '%$val%' OR Item_name LIKE '%$val%' ORDER BY Item_number ASC  LIMIT $offset,$record_show";
             $state = '5';
        }
        else if(($val !='' AND $Mfg_val =='MFG' AND $Status_val !='STATUS')){
            $sql = "SELECT * FROM tbl_spare_part
            ...
            $sql = "SELECT * FROM tbl_spare_part
            WHERE Item_number LIKE '%$val%' OR Item_name LIKE '%$val%' AND
                  Mfg_id LIKE '%$Mfg_val%' ORDER BY Item_number ASC  LIMIT $offset,$record_show";
                   $state = '7';
        }
        else if(($val !='' AND $Mfg_val !='MFG' AND $Status_val !='STATUS')){
            $sql = "SELECT * FROM tbl_spare_part
