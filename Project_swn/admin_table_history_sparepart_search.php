...
?>
  ...
  <?php
        ...
        if($val =='' AND $Mfg_val =='MFG'){
            $sql = "SELECT * FROM tbl_borrow_sparepart ORDER BY B_id ASC  LIMIT $offset,$record_show" or die("Error:" . mysqli_error());

        }
        else if(($val =='' AND $Mfg_val !='MFG' )){
            $sql = "SELECT * FROM tbl_borrow_sparepart
            WHERE B_Mfg LIKE '%$Mfg_val%' ORDER BY B_id ASC   LIMIT $offset,$record_show";
 
        }
        else if(($val !='' AND $Mfg_val =='MFG')){
            $sql = "SELECT * FROM tbl_borrow_sparepart
            WHERE B_user LIKE '%$val%' OR B_userID LIKE '%$val%' OR B_Item_number LIKE '%$val%' OR B_Item_name LIKE '%$val%' 
            OR date_borrow LIKE '%$val%'OR date_return LIKE '%$val%'OR date_return_True LIKE '%$val%' ORDER BY B_id ASC  LIMIT $offset,$record_show";
           
        }
        else if(($val =='' AND $Mfg_val !='MFG' AND $Status_val !='STATUS')){
            $sql = "SELECT * FROM tbl_borrow_sparepart
            WHERE B_user LIKE '%$val%' OR B_userID LIKE '%$val%' OR B_Item_number LIKE '%$val%' OR B_Item_name LIKE '%$val%' 
            OR date_borrow LIKE '%$val%'OR date_return LIKE '%$val%'OR date_return_True LIKE '%$val%' AND  B_Mfg LIKE '%$Mfg_val%' ORDER BY B_id ASC  LIMIT $offset,$record_show";
           }
                    ...
                    <div class="container-fluid px-4">
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                ...
                                <form action="admin_table_history_sparepart_search.php" class="form-group" method="POST">
